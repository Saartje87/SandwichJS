...
PB.Class2 = function ( parentClass, baseProto ) {

	var child, childProto, constructor,
		name, prop, parentProp;
	...
	if( parentClass ) {
		...
		constructor = parentClass.constructor;
	}

	child = constructor
		? function () { return constructor.apply(constructor, arguments); }
		: function () {};
	...
	for( name in baseProto ) {
		...
		if( baseProto.hasOwnProperty(name) && name !== 'construct' ) {
			...
			prop = baseProto[name];
			parentProp = parentClass ? parentClass[name] : null;

			console.log(prop, parentProp);

			if( typeof prop === 'function' && typeof parentProp === 'function' ) {
	...
	PB.extend(childProto, parentClass);
